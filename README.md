...
The host can upload a landscape JPG, PNG or still WebP in campaign settings, up to 512 KB and 2048 pixels per side. It replaces the default harbor or abstract backdrop, allowing the visual setting to fit a different era. Scene reads check campaign membership and only the host can change or restore the backdrop, even if the host has no player character. Changing artwork does not change location, discoveries, resources or turns. Hosts can also generate and review a landscape preview when scene generation is enabled. Backdrops do not update automatically after travel.
...
Enable `LANTERN_SCENES_ENABLED=true` to let hosts use Campaign settings → Scene artwork → Generate scene artwork. This uses the saved world and current location only; private notes, campaign premise, chat and story history are excluded. The same image model produces one 1536×1024 medium-quality compressed JPEG, limited to 512 KB. Portraits and scenes share the existing image allowance and per-account lease.

Generation returns a temporary preview. The host chooses Use generated scene to save it privately for campaign members. Closing the panel loses the preview. If the saved world or location changes during generation, the response is rejected and the preview panel resets. The current host is checked again before returning the image, and scene saving independently checks host permission. Artwork does not move the party, reveal journal entries or spend character resources. Failed, discarded or interrupted requests can still consume image allowance and provider usage. Scene generation is disabled by default for self-hosters; the private Railway playtest enables it.
