...
import { readScene, uploadScene } from '../../../lib/game/service';
...
export async function GET(request: Request) {
  try {
    ...
    if (!user) throw new GameError('Sign in to view scene artwork.', 401);
    const u = new URL(request.url);
    const object = await readScene(
      user,
      text(u.searchParams.get('campaign'), 'Campaign', 100),
      text(u.searchParams.get('asset'), 'Scene', 100),
    );
...
export async function POST(request: Request) {
  try {
    ...
    const user = await currentUser();
    if (!user) throw new GameError('Sign in to upload scene artwork.', 401);
    const id = text(
