...
| Open-source project | MIT license, contribution guide, reproducible clean-source export and tested downloadable source snapshot; standalone build and hosting guide | Public GitHub publication is pending. The previously supplied ZIP is a snapshot and does not automatically include later changes. Hosting portability supports Cloudflare and Railway with persistent SQLite/private images. |
